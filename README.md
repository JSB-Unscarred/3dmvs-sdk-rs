...
| 文件传输 | `MV3D_LP_FILE_ACCESS` 输入描述符只在启动调用期间读取；后台至多保留其中的字符串指针；只有 `completed == total > 0` 表示完成 | 描述符保持调用期有效；文件名只保留到观察到完成或设备清理，之后立即释放且不累计历史；若发现 SDK 保留描述符本身，则停用并重新设计该 safe API |
